...
if __name__ == '__main__':
    ...
    with open('train_stats.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["epoch", "reward", "epsilon", "time_elapsed", "frame_processed", "negative_reward_counter"])
        ...
        for e in range(STARTING_EPISODE, ENDING_EPISODE+1):
            ...
            while True:
                ...
                if done or negative_reward_counter >= 25 or total_reward < 0:
                    ...
                    gc.collect()
                    tf.keras.backend.clear_session()
                    epoch_finished_time = time.time()
                    writer.writerow([e, float(total_reward), float(agent.epsilon), round(epoch_finished_time - start_time), time_frame_counter, negative_reward_counter])
                    break
